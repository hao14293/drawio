...
![批量添加布控-流程图.drawio](%E5%AD%A6%E4%B9%A0%E7%AC%94%E8%AE%B0.assets/%E6%89%B9%E9%87%8F%E6%B7%BB%E5%8A%A0%E5%B8%83%E6%8E%A7-%E6%B5%81%E7%A8%8B%E5%9B%BE.drawio.png)
...
![删除布控-流程图.drawio](%E5%AD%A6%E4%B9%A0%E7%AC%94%E8%AE%B0.assets/%E5%88%A0%E9%99%A4%E5%B8%83%E6%8E%A7-%E6%B5%81%E7%A8%8B%E5%9B%BE.drawio.png)
...
![image-20240809094851664](%E5%AD%A6%E4%B9%A0%E7%AC%94%E8%AE%B0.assets/image-20240809094851664.png)
...
![人像库检索和查询-流程图.drawio](%E5%AD%A6%E4%B9%A0%E7%AC%94%E8%AE%B0.assets/%E4%BA%BA%E5%83%8F%E5%BA%93%E6%A3%80%E7%B4%A2%E5%92%8C%E6%9F%A5%E8%AF%A2-%E6%B5%81%E7%A8%8B%E5%9B%BE.drawio.png)
...
3. 如果下发的参数中，有图片信息
   ...
   ​		通过检索结果的memberId，去查询云库，将结果最终补齐返回
...
![车辆-添加以图搜图任务.drawio](%E5%AD%A6%E4%B9%A0%E7%AC%94%E8%AE%B0.assets/%E8%BD%A6%E8%BE%86-%E6%B7%BB%E5%8A%A0%E4%BB%A5%E5%9B%BE%E6%90%9C%E5%9B%BE%E4%BB%BB%E5%8A%A1.drawio.png)
...
* 请求内容
  ...
  ```json
  {
      "image": {
          ...
          "regionRects": [
          {   // 车辆区域内的局部坐标集合，object类型为绝对坐标点，以原图左上角为起点。可选，算法暂不支持。
              "leftTopX": 101,  // 左上横坐标
              "leftTopY":101,
              "rightBtmX":201,
              "rightBtmY":201
      ...
      "feature": "QWESADASDASDASDAASDASD", // 特征值
      "order" : {"similarity" : "-" }, // 根据时间(time)或者相似度(similarity)排序。"-" 代表降序，"+" 代表升序。非必填，默认按相似度降序。
      "channelIds": ["123"],   // PaaS层通道编号列表。
      "startTime": "20180101T000000Z",  // 起始时间
      "endTime": "20180101T000000Z",    // 终止时间。
      "vehicle": {  // 车辆对象, 2.1版本废弃
          "plateNum": "浙A12345",
          "vehicleBrands": ["1","2"],
          "vehicleMode": "1001",
          "vehicleStyle": "2018",
          "vehicleColors": ["1","2"],
          "vehicleTypes": ["k10","K12"],
          "snapHeadstock":0,
          "carAttribute":"A",
          "hasHangings": 0,
          "hasDetectionSign": 0,
      ...
      "threshold": 0.90,
      "resultNum": 1000, // 期望返回的条数。可选，默认值500，最大值1000。
      "targetType": 1,  // 类型int。 可选，可按位组合，全1表示全检测,1表示人脸，2表示人体，4表示机动车，8表示非机动整体, 16表示非机动车（人骑车扣车）, 默认0或无该字段表示全图检测
      "algorithmVersion": "1001001002003" // 特征模型版本；特征向量数据(feature)不为空时, 必填。
  }
  ```
...
```json
{
    "taskId": "xxxxxx"  // 以图搜图检索任务id
}
```

* 注意事项、

1. 新增以图搜图任务时，通道不存在则taskId返回-1。
2. searchAttr优先级高于vehicle



**整体逻辑与 11.添加以图搜图任务一致，提取特征时多了框，但在业务代码里并没有区别，只是将框坐标设置到图片信息里，由算子去处理。**

### 15. 删除以图搜图任务----(不再使用)

```java
// 删除异步以图搜图任务。
DELETE /processing/retrieval/vehicle/[taskId]
```

![车辆-删除以图搜图任务-流程图.drawio](%E5%AD%A6%E4%B9%A0%E7%AC%94%E8%AE%B0.assets/%E8%BD%A6%E8%BE%86-%E5%88%A0%E9%99%A4%E4%BB%A5%E5%9B%BE%E6%90%9C%E5%9B%BE%E4%BB%BB%E5%8A%A1-%E6%B5%81%E7%A8%8B%E5%9B%BE.drawio.png)

### 16. 检测图片中的车辆

```JAVA
// 用户上传图片，检测图片中所能检测出来的车辆，并返回结构化信息以及相关的坐标信息
POST /processing/detect/vehicle
```
...
* 请求内容
  ...
  ```json
  {
      "data": "xxxxx",   // 图片数据的base64编码。 如果携带图片，则是图片数据，与uri字段互斥
      "uri": "xxxxxx"    // 图片uri 设备需要到此地址去下载图片做分析。
  }
  ```
...
```java
// 添加一路图片分析,可指定分析算法版本。
POST  /processing/analysis/vehicle/channels/[channelId]
/**
{
    "type": 1,                 //分析类型，1 表示分析图片通道
    "regionId":"XXX",          // 域ID。
    "faceEnable":false,       // true开启人脸通道分析，false 不开启。非必填（无该字段表示不开启）。
    "memo": "我的通道"          //备注信息，最长128字符  
}
*/
// 响应参数
...
```java
// 通过检测，或者用户选择的矩形框搜索相似的图片
POST /processing/retrieval/extractFeature
```

* 注意事项

  > 要和获取异步检索结果-SaaS补齐配对调用
  >
  > image::data、image::uri、feature 、recordId 四者必选其一
  >
  > 使用优先级顺序：feature >image::uri>image::data>recordId
...
* 请求参数
  ...
  ```json
  {
      "threshold": 0.85, 
      "resultNum": 100,
      "startTime":"20171215T144307Z",
      "endTime":"20171215T144307Z",
      "channelIds":["123", "456"],
      "retrievalType":1, // 查询的数据流类型。0 表示图片流/实时流类分析数据；1 表示中心录像/离线视频任务类分析数据。选填项，默认为0。
      "image": {
          "data": "xxxx",
          "uri": "",
          "rect": {
          ...
          "extractNoDetect": 0 // 是否强提特征;0表示不进行强提特征，1表示根据输入的坐标框进行强提特征不做目标检测(该值为1时，image::rect必填)
      },
      "feature": "xxxxx", // 如果有feature，则检测以feature优先。与image有且仅有一个。
      "recordId": "xxxxx", // 如果有feature，则检测以feature优先。与image有且仅有一个。
      "objectType": 1, // 人 1;机动车	2;非机动车 3;人脸	4; 局部 5
      "searchAttr":{  // 图搜过滤条件。(自身关联也为1)
          "relatedFace": 1, // 关联人脸，0-不关联，1-关联 (自身关联也为1)
          "relatedPerson": 1,
          "relatedNonVehicle": 0,
          "relatedVehicle": 0,
          "releatedGait": 0
...
> 冷搜任务是指特征存储在磁盘中，需要实时加载到内存中进行比对，整个过程比较耗时。

### 1. 添加冷搜任务

```json
// 通过接口创建一个冷搜任务
POST /processing/cold/retrieval
```
...
* 请求参数
  ...
  ```json
  {
      "taskId":"123",  // 任务ID,可以由外层传入，为空系统自动生成
      "businessType":1, 	// 0	人脸
                          // 1	车辆
                          // 2	结构化
                          // 3	场景认知
      ...
      "startTime":"20171215T144307Z",
      "endTime":"20171215T144307Z",
      "channelIds":["123", "456"], // 待搜索的通道列表。非必填，默认代表所有通道。
      "retrievals": [{ // 至少有一个内容，最大支持32组数据。
          "retrievalId": "xxx", // 检索号。必填项。用来标识每个以图搜图的编号，与返回结果的retrievalId 对应。
          "image": {
              "data": "xxxx",
              "uri": "",
              "rect": {
...
1)   根据jobId查询codeRetrievalJob表，判断是否已经存在此任务

2)   支持批量32，且限流 50个job任务

3)   判断检索时间段，如果只有热搜时间，则返回失败

4)   获取对应类型的分析算子

会首先获取双模型算子，没有双模型算子，再找分析算子

5)   进行特征提取

6)   将特征存储到PF 

双模型，两个特征均保存；多个目标，保存返回的第一个特征

7)   通道信息处理，只找对应类型的，实时分析通道，得到绑定的库Ids

8)   处理搜索时间，切分为冷搜时间段 与 热搜时间段

9)   持久化记录到codeRetrievalJob表，状态设置为waitting

10) 发布冷搜事件  

11) 返回结果

##### 1.3.1  发布冷搜事件后的处理逻辑 （onApplication 进行监听）

1)   State 初始时为waitting,先执行**热搜**

2)   根据sql中存储的信息，先去PF下载得到特征

3)   查询库-资源关系，得到<算子，List<库Ids>>关系

这一步注意，会查询离线的算子，直接抛出异常了

4)   请求算子进行检索，且等待结果返回

5)   将结果写入Redis,Sorted Set

Key为jobId_retrievalId

6)   处理检索结果，从redis中查询出结果，取topN

7)   结果返回后发送mq到云库保存，将数据库中的任务处理状态改为热搜处理完成状态



1)   State若为HOT_PHASE_FINISHED,则添加**冷搜任务**

2)   先根据库Ids 查询sql 得到绑定的冷搜算子信息

然后得到<算子，List<库Ids>>关系

3)   生成冷搜的Task任务，写入coldRetrievalTask表

4)   异步执行冷搜开启任务

5)   数据库中的任务处理状态改为COLD_PHASE_RUNNING

##### 1.3.3 **异步执行冷搜开启任务处理逻辑** （提交线程池 看run()）

1)   加锁

2)   根据任务id从数据库查询任务详细信息，判断状态

如果是等待状态，则可以执行后续逻辑

3)   获取冷搜算子，判断冷搜能力是否充足,

能力不足或者为pedding状态则继续放入队列等待排队重试，

如果算子状态时缩容的则直接将任务状态置为异常，错误信息记录为无冷搜资源

4)   如果算子为运行状态且能力足够则将冷搜任务下发给冷搜算子

5)   如果冷搜算子返回失败，则继续放入队列等待排队重试

##### 1.3.4 **下发冷搜任务后，算子进度上报**

1)   下发冷搜任务使用的是单向流，进度上报用了另一个双向流
 ColdRetrievalEventNotifyStreamObserver

2)   加锁

3)   根据taskId查询分片任务信息, 判断状态，只有数据库中任务状态为运行中3的任务才允许修改

4)   按照算子上报的参数更新数据库

5)   判断，如果状态为error或者finish 则需要关闭任务

##### 1.3.5  **定时扫描，处理冷搜任务**

冷搜还会通过定时线程，三分钟扫描一次，来处理任务。ColdPhaseScanTableScheduler

1)   查询sql coldRetrieval表，状态为waitting或者HOT_PHASE_FINISHED的记录

2)   发布冷搜事件，观察者收到后，执行冷搜处理逻辑

##### 1.3.6    **定时扫描，合并冷搜任务**

冷搜还会通过定时线程，五分钟扫描一次，处理已经下发给算子的任务。

1)   查询sql，ColdRetrievalJob状态为COLD_PHASE_RUNNING的记录，即已经下发给算子执行冷搜任务了。

2)   查询sql , ColdRetrievalJob下对应的任务 ColdRetrievalTask都完成的job

3)   更新ColdRetrievalJob,如果Task存在error状态，则更新job为exception状态；无error,则更新状态为finished

4)   处理每个Task返回的检索结果，合并取topN 

5)   将结果发mq 给云库进行保存

一直发送失败，会将job状态置为exception



## 十一、 NVN

逃犯经常更换身份证，被添加在其他常口库中，通过逃犯库和常口库对比，从而找到此逃犯。NVN功能就是两个库之间的特征值比较，找到相识度大于某个阈值的用户。
...
Sass -> Perception: 1: 下发创建NVN任务，提供源库与被对比库信息
Perception -> Perception: 库与通道参数检验
Perception --> Sass: 参数异常
Perception -> Perception: 检验NVN任务是否存在
Perception --> Sass: 任务已存在异常
Perception -> Perception: 根据对比库类型不同，组建不同的NVN任务请求（静态、抓拍）
Perception -> Mysql: 查询正在运行的NVN任务数量，判断是否满足NVN能力集
Mysql --> Perception: 
Perception -> Mysql: 将NVN任务插入NVN_JOB表
Mysql --> Perception: 
Perception -> Perception: 生成Task任务
Perception -> Mysql: 将task信息插入Task表
Mysql --> Perception: 
Perception -> Perception: task任务放入NVN task队列
Perception --> Sass: 创建成功，返回NVN任务ID
note over Perception,检索算子: NVN Task监听线程
Perception -> Perception: 从NVN task队列获取待执行的NVN task
Perception -> Mysql: 确认该task在数据库中存在
Mysql --> Perception: 
Perception -> Mysql: 由待对比库、源库信息查询库、库资源表，获取检索算子编号
Mysql --> Perception: 
Perception -> RM: 由算子编号查询检索算子信息
RM -> Perception:
Perception -> 检索算子: 创建NVN任务
检索算子 --> Perception: 
note over Perception,检索算子: NVN Task线程结束
```
...
![image-20240904105057451](%E5%AD%A6%E4%B9%A0%E7%AC%94%E8%AE%B0.assets/image-20240904105057451.png)

* NVN任务创建异步线程



<img src="%E5%AD%A6%E4%B9%A0%E7%AC%94%E8%AE%B0.assets/image-20240904105118606.png" alt="image-20240904105118606" style="zoom:200%;" />

#### 1.3 流程描述

1. 对请求创建NVN相关的库或通道校验，判断是否存在

2. 判断NVN任务是否已经创建

3. 组建创建NVN请求，任务信息持久化入NVN_JOB

4. 以源库为基准，生成task任务信息，持久化入TASK表

5. task入队列排队，返回给saas平台创建的NVN任务ID

6. 异步线程实时扫描task队列，获取源库对应算子资源信息，与待对比库对应算子编号，向源库对应算子资源创建NVN任务
...
Sass -> Perception: 下发删除NVN任务
Perception -> MySQL: 获取NVN任务详细信息
MySQL --> Perception: 
Perception --> Sass: 不存在该任务，直接返回成功
Perception -> MySQL: 查询TASK表，获取该任务下所有TASK状态信息
MySQL --> Perception: 
Perception -> MySQL: 修改TASK状态为逻辑删除-5
MySQL --> Perception:
Perception -> Perception: 将task排队进入删除队列
note over Perception,检索算子: NVN task 监听线程
Perception -> MySQL: 该任务若无所属任何算子资源的记录或算子离线，直接删除TASK表信息
MySQL --> Perception:
Perception -> RM: 根据task记录的算子id，获取算子信息
RM -> Perception: 
Perception -> 检索算子: 下发关闭NVN任务
检索算子 --> Perception: 
note over Perception,检索算子: NVN task 监听线程结束
Perception -> MySQL: 删除NVN_JOB记录
MySQL --> Perception: 
Perception --> Sass: 返回成功200
```
...
2. 判断该任务是否存在后，获取该任务下所有task信息，修改状态为逻辑删除
...
Sass -> Perception: 启停NVN任务
Perception -> Perception: 参数检验
Perception -> MySQL: 根据NVN任务ID，查询NVN_JOB和TASK表，获取未完成的NVN任务
MySQL --> Perception: 
Perception -> Perception: 若为开启操作，判断能力集
Perception -> MySQL:根据任务ID，查询TASK表，更新对应task状态为waitting
MySQL --> Perception: 
Perception -> Perception: 添加task到排队队列
note over Perception,MySQL: NVN task 监听线程
Perception -> Perception: 判断是启动还是停止，向算子开启或关闭分析任务
note over Perception,MySQL: NVN task 监听线程结束
Perception -> MySQL: 更新NVN任务的enable状态
MySQL --> Perception: 
Perception --> Sass: 返回成功200
```
...
1. Saas平台带NVN任务id请求启动/停止任务

2. 由任务ID，联表查询NVN_JOB、TASK表 ，获取未完成状态的TASK表记录并关联NVN_JOB表记录

3. 若平台请求为启动NVN任务，那么要判断是否满足能力集

4. 更新TASK表记录STATUS字段 未完成状态->等待中，并将任务排入等待队列，供队列监听线程执行任务下发
...
Sass -> Perception: 查询NVN任务
Perception -> MySQL: 查询NVN_JOB表，获取相关任务信息
MySQL --> Perception: nvnJob信息
Perception -> MySQL: 根据任务ID查询包含的所有的task信息
MySQL --> Perception:
Perception -> Perception: 汇总task进度，计算总体任务进度
Perception --> Sass: 任务进度
```
...
1. saas带NVN任务id向感知服务请求获取相应任务信息。若不带任务id，则为分页查询所有NVN任务；

2. 查询NVN_JOB表，得到NVN任务的相关信息如创建时间、优先级等

3. 根据job_id查询TASK表中任务类型为NVN的task信息

4. 根据job下所有task的任务进度process、状态status，得到job的进度与状态

5. 将汇总的所有job信息响应saas请求

### 5. NVN任务管理

#### 5.1 资源管理中心上线

1. 订阅ResourceManager服务，服务上线后触发。

2. 定时，每10分钟执行一次

> 服务先起来，RM上线后会触发WholeSyncNvnListener，会执行定时线程，就会定时批量同步算子和内存中的任务



```mermaid
graph TD
	a1(WholeSyncNvnListener)-->b1["向资源管理中心请求进行中、加载库状态得算子资源"]
	b1-->b2["向每一个算子请求，获取其上运行得NVN任务operatorList"]
	b2-->b3["查询task表中running状态的NVN任务列表taskList"]
	b3-->b4["将taskList中存在而operatorList不存在的任务，重新修改为waiting状态并放入task执行队列"]
	b4-->b5["将operatorList中存在而taskList不存在的任务关闭"]
	b5-->e(end)
```
...
```mermaid
graph TD
	a("SyncNvnListener")-->b1["从资源管理中心获取算子信息"]
	b1-->b2["查询该算子上的NVN任务信息operatorList"]
	b2-->b3["查询TASK表上该算子的状态为running的NVN任务列表taskList"]
	b3-->b4["将taskList中存在而operatorListn不存在的task任务，重新修改为waiting状态并放入task执行队列"]
	b4-->b5["将operatorList中存在而taskList不存在的task任务关闭"]
	b5-->e("end")
```
...
```mermaid
graph TD
	a1("MoveNvnListener")-->b1["从资源管理中心获取算子信息"]
	b1-->b2{"算子状态为运行或加载中"}
	
	b2--No-->b3["查询该算子上任务状态为running的NVN任务列表taskList"]
	b3-->b4["将taskList中的任务修改为waitting并放入task执行队列"]
	b4-->e("end")
	b2--Yes-->e
```
...
检索算子缩扩容操作，会影响NVN任务迁移，定时每10秒检测一下算子缩扩容操作有无产生
...
![算子缩扩容整体流程图.drawio](%E5%AD%A6%E4%B9%A0%E7%AC%94%E8%AE%B0.assets/%E7%AE%97%E5%AD%90%E7%BC%A9%E6%89%A9%E5%AE%B9%E6%95%B4%E4%BD%93%E6%B5%81%E7%A8%8B%E5%9B%BE.drawio.svg)

### 2. 算子缩扩容后的操作

```java
public class RebalanceResourceManager {
    @Scheduled(fixedDelay = 60000)
    public void rebalanceResource() {
        //note grpc 请求扩容和缩容的资源变动请求信息 ResourceChangeTasksInfo
        QueryResourceChangeTasksResponse response = resourceManagerRpcStub.queryResourceChangeTasks(request);
        List<ResourceChangeTasksInfo> list = response.getResourceChangeTasksInfoList();
        //...
        try {

            //缩容操作
            reduceResource(list.stream().filter(task -> RebalancedTypeEnum.BALANCE_TYPE.getType().equals(task.getType())
                    && BALANCE_RESOURCE_TYPE.contains(task.getResourceType())).collect(Collectors.toList()));

            //扩容操作
            dilatationResourceManager.dilatationResource(list.stream().filter(task -> RebalancedTypeEnum.DILATATION_TYPE.getType().equals(task.getType())
                    && BALANCE_RESOURCE_TYPE.contains(task.getResourceType())).collect(Collectors.toList()));

            //NVN任务迁移
            if (list.stream().filter(task -> FACE_RETRIEVAL_MATCH_TYPE.getType().equals(task.getResourceType())).findAny().isPresent()) {
                applicationEventPublisher.publishEvent(new NVNTaskMoveEvent(new Object()));
            }
        }
    }
}
...
#### 2.1 taskManager缩容 reduceResource

总体来说，将需要缩容的算子上绑定的库卸载掉，然后绑定到同类型的其它算子上，需要判断算子的剩余能力集。

![算子缩容后的操作.drawio](%E5%AD%A6%E4%B9%A0%E7%AC%94%E8%AE%B0.assets/%E7%AE%97%E5%AD%90%E7%BC%A9%E5%AE%B9%E5%90%8E%E7%9A%84%E6%93%8D%E4%BD%9C.drawio.svg)

##### 缩容整体流程描述

1. web当点击缩容时，首先调用mplatform验证此次操作是否支持缩容；

2. web需要限制聚类算子以及归档算子不支持缩容；并且归档算子个数只能为1个

3. mplatform判断是否能够缩容规则如下：

   a)   包含不可缩容类型的算子：聚类算子与归档算子

   b)   如果缩容算子中包含检索类算子，则需要调用taskManager服务验证是否可以缩容，参数中带缩容算子的算子编号；taskManager会将缩容算子上的黑白名单库以及静态库按照能力集重新分配，验证缩容后算子加载库能力是否满足库特征总量，如果不满足则返回失败；

4. Web在mplatform返回后根据结果显示当前缩容是否支持，二次确认，如果用户仍想要缩容，则点击强制缩容按钮，web调用mplatform缩容接口进行缩容；如果不进行缩容操作则点击取消；

5. Mplatform接收到缩容请求后，按正常缩容流程下发资源管理服务

6. 资源管理服务受到缩容请求后，进行缩容操作，缩容完成之后，如果存在比对类算子的缩扩容操作则将缩容结果写入redis； 

7. taskManager服务定时通过gRPC查询缩扩容记录，如果存在缩扩容记录则执行对应重新负载库的操作

8. taskManager进行推库或卸载库的操作

9. taskManager同步路由给executor
...
mp -> RM: 缩容
RM -> mysql: 任务写入数据库TaskQueue表
mysql --> RM:
RM --> mp:
note over mp,agent: 后台线程
RM -> mysql: 查询taskQueue表中的缩容任务
mysql --> RM:
RM -> RM: 选择需要缩容的算子
RM -> mysql: 修改resource表中的对应资源状态
mysql --> RM:
RM -> mysql: 缩容记录写入RESOURCE_CHANAGE_TASK
RM -> agent: 关闭算子
```
...
1. web下发扩容请求，页面控制归档算子只能启动一个，其他算子根据资源量启动

2. mplatform调用资源管理服务接口进行扩容；

3. 资源管理保存任务，后台线程选择agent节点启动算子，如果存在比对类算子的缩扩容操作则将扩容结果写入redis；

4. taskManager服务定时通过gRPC查询缩扩容记录，如果存在缩扩容记录则执行对应重新负载库的操作

5. taskManager进行推库或卸载库的操作

6. taskManager同步路由给executor

#### 2.3 算子缩扩容对NVN任务的影响

![算子缩扩容对NVN任务的影响.drawio](%E5%AD%A6%E4%B9%A0%E7%AC%94%E8%AE%B0.assets/%E7%AE%97%E5%AD%90%E7%BC%A9%E6%89%A9%E5%AE%B9%E5%AF%B9NVN%E4%BB%BB%E5%8A%A1%E7%9A%84%E5%BD%B1%E5%93%8D.drawio.svg)

## 十三、潮汐调度

潮汐调度是指在某段时间内，某种业务耗费资源能力减少，此时可以将该业务占用的资源进行共享给与需要使用算力资源的业务。

当前潮汐调度只支持**人脸，车辆，结构化**的**图片分析业务**的算力资源共享给**结构化视频分析业务**，需要**搭配预案任务**使用。

**中台服务**负责**潮汐配置的增删改**；

**资源管理服务**负责根据**潮汐策略潮汐算子**

Perception服务负责感知潮汐配置以及预案的变化，生成不同的潮汐策略，并将潮汐策略同步给资源管理中心；

1.潮汐调度支持配置潮汐时间段，算力共享，定时潮汐，定时回滚
...
service AgentService {
    rpc startTask(TaskStartRequest) returns (TaskStartResponse){}
    rpc stopTask(TaskStopRequest) returns (TaskStopResponse){}
    rpc operatorPackage(OperatorPackageRequest) returns (OperatorPackageResponse){}
    rpc getLocalAlgorithmPackagePath(AlgorithmPackageRequest)returns(AlgorithmPackageResponse){};
    rpc algoPackage(OperatorPackageRequest) returns (AlgoPackageResponse){}
    rpc modelPackage(OperatorPackageRequest) returns (ModelPackageResponse){}
    rpc thirdParPackage(OperatorPackageRequest) returns (ThirdParPackageResponse){}
}
```
...
```protobuf
service ResourceService {
    rpc queryResourceById (QueryResourceByIdRequest) returns (ResourceInfosResponse){}
    rpc queryResource (QueryResourceRequest) returns (ResourceInfosResponse){}
    rpc queryConfigRule (QueryConfigRuleRequest) returns (ConfigRuleInfosResponse){}   //查询智能配置规则信息,行为分析算子用到
    rpc queryCapacitySummary (QueryCapacitySummaryRequest) returns (CapacitySummaryResponse){}  //查询智能能力概要信息
    rpc queryPowerDetail (QueryPowerDetailRequest) returns (PowerDetailResponse){}    //根据条件查询算力详细信息
    rpc queryResourceBySerialNum (QueryResourceBySerialNumRequest) returns (ResourceInfosResponse){} //根据算子编号列表查询资源信息列表,20201127新增协议
    rpc queryResourceDetailByPage (QueryResourceDetailByPageRequest) returns (QueryResourceDetailByPageResponse){} //20201127http转grpc，/processing/zeus/resource/details
    rpc queryCapacityDetailBySerialNum (QueryCapacityDetailBySerialNumRequest) returns (QueryCapacityDetailBySerialNumResponse){}  //20201127restFul转grpc，获取指定算子序列号的能力集详情信息 GET /processing/zeus/resgister/capacityDetail/${serialNum}
    rpc queryResourceScreenInfo (ResourceScreenInfoRequest) returns (ResourceScreenInfoResponse){}    //20201127restFul转grpc，获取资源过滤信息接口 GET /processing/zeus/resource/screenInfo
    rpc querySupportAlgorithmList (QuerySupportAlgorithmListRequest) returns (QuerySupportAlgorithmListResponse){}    //获取系统当前支持的可调度资源的所有算法信息
    rpc queryEdgeResourceByOperatorId(QueryEdgeResourceByOperatorIdsRequest) returns (ResourceInfosResponse){}  //只查询边端资源，不查询云端
    rpc queryStopResource(QueryResourceRequest) returns (ResourceInfosResponse){} //IVSS数据迁移需要查询缩容算子
    rpc queryResourceChangeTasks(QueryResourceChangeTasksRequest) returns (QueryResourceChangeTasksResponse){}  // 查询缩扩容记录
    rpc deleteResourceChangeTasks(DeleteResourceChangeTasksRequest) returns (DeleteResourceChangeTasksResponse){}  // 删除缩扩容记录
    rpc queryResourceDetailByPowId(QueryResourceByPowIdRequest) returns (QueryResourceDetailByPageResponse){}  //V2.3新增，扩缩容需要
    rpc insertResourceByType(InsertResourceRequest) returns (InsertResourceResponse){}
}
```
